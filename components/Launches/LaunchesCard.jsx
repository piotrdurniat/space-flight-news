...
const LaunchesCard = ({
                      launch: { name, pad, net, mission, rocket},
                    }) => {
  const dateStr = new Date(net);
  const currentDate = new Date();
  const timeToStart = dateStr.getTime() - currentDate.getTime();
  let imageUrl;
  (rocket.configuration.manufacturer.image_url) ? imageUrl =
    rocket.configuration.manufacturer.image_url : imageUrl = "https://images.unsplash.com/photo-1517976487492-5750f3195933?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1170&q=80";
  ...
  return (
    <Card style={{ marginBottom: 16 }}>
      ...
      <Card.Content>
        ...
        <Text style={{ textAlign: "center", marginBottom: -10 }} variant="labelMedium">{`Time to start:`}</Text>
        <CountDownTimer time={timeToStart} />
        <Text style={{ textAlign: "center", marginTop: -10, marginBottom: 6 }}
              variant="labelMedium">{`Date of launch:`}</Text>
      ...
      <Card.Actions style={styles.container}>
        ...
        {wikiUrl
          ? < Button onPress={() => Linking.openURL(wikiUrl)}>Show pad info </Button>
          : null
        }
      </Card.Actions>
    </Card>
